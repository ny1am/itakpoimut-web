...
import {
  clearFilters, changeLoyalty, changeCategory, removeViolationFilter
} from 'actions/companies';
import loyalties from 'utils/enums/loyalty';
import SelectedFiltersComponent from './SelectedFilters';

const getSelectedLoyalty = state => loyalties.find(
  l => l.name===state.companies.selectedLoyalty
);
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
  ...
  const {
    onLoyaltyChange, onCategoryChange, onRemoveViolationFilter, onClearFilters
  } = dispatchProps;
  const filters = [];
  selectedLoyalty && filters.push({
    text: selectedLoyalty.text,
    onRemove: () => onLoyaltyChange(null)
  });
  selectedCategory && filters.push({
    text: selectedCategory.text,
    onRemove: () => onCategoryChange(null)
  });
  filters.push(...selectedViolations.map(violation => ({
    text: violation.text,
